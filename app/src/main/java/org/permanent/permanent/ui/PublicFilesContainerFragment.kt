...
class PublicFilesContainerFragment : PermanentBottomSheetFragment() {
    ...
    private var publicFilesFragment: PublicFilesFragment? = null
    private var selectedDestinationRecord: Pair<Workspace, Record?> = Pair(Workspace.PUBLIC_FILES, null)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        publicFilesFragment = PublicFilesFragment()
        publicFilesFragment?.getOnRecordSelected()?.observe(this, onRecordSelectedObserver)
        publicFilesFragment?.arguments = bundleOf(SHOW_SCREEN_SIMPLIFIED_KEY to true)
        val transaction: FragmentTransaction = childFragmentManager.beginTransaction()
        transaction.replace(R.id.frameLayoutContainer, publicFilesFragment!!).commit()
    }
    ...
    override fun disconnectViewModelEvents() {
        ...
        publicFilesFragment?.getOnRecordSelected()?.removeObserver(onRecordSelectedObserver)
    }
